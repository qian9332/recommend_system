...
class NegativeSamplerV2:
    """全向量化负采样器：消除所有Python循环"""
    
    def __init__(self, num_items, user_inc_items, user_positive_items, num_users, device):
        self.num_items = num_items
        self.device = device
        
        # 优化：预构建INC tensor（每个用户最多保存100个INC商品）
        print("构建INC tensor...")
        max_inc_per_user = 100
        self.inc_tensor = torch.zeros(num_users, max_inc_per_user, dtype=torch.long, device=device)
        self.inc_counts = torch.zeros(num_users, dtype=torch.long, device=device)
        
        for uid, inc_list in tqdm(user_inc_items.items(), desc="构建INC"):
            if uid < num_users:
                count = min(len(inc_list), max_inc_per_user)
                self.inc_tensor[uid, :count] = torch.tensor(inc_list[:count], dtype=torch.long)
                self.inc_counts[uid] = count
    ...
    def inc_negative_vectorized(self, user_ids, num_neg, device):
        """INC负采样 - 向量化版本"""
        ...
        # 批量随机选择INC商品
        random_indices = torch.randint(0, 100, (batch_size, num_neg), device=device)
        ...
        for i in range(batch_size):
            ...
            if count >= num_neg:
                # 随机选择num_neg个
                perm = torch.randperm(int(count), device=device)[:num_neg]
                result[i] = self.inc_tensor[uid, perm]
            else:
                # 用已有的INC + 随机补充
                if count > 0:
                    result[i, :count] = self.inc_tensor[uid, :count]
                # 补充随机
                result[i, count:] = torch.randint(1, self.num_items, (num_neg - count,), device=device)
        
        return result
    
    def inbatch_negative_vectorized(self, item_ids, num_neg):
        """In-Batch负采样 - 向量化版本"""
        ...
        # 方法：使用随机排列从batch中选择
        # 生成随机排列矩阵
        perm = torch.randperm(batch_size, device=device).unsqueeze(0).expand(batch_size, -1)
        
        # 排除自己：将每个位置设为一个不可能的值，然后重新排列
        # 简化方法：直接用随机+排除
        result = torch.zeros(batch_size, num_neg, dtype=torch.long, device=device)
    ...
    def hard_negative_vectorized(self, user_vectors, all_item_vectors, user_ids, num_neg, device):
        """难负例挖掘 - 向量化版本"""
        ...
        # 批量TopK [B, K]
        top_k = similarities.topk(num_neg * 2, dim=1).indices + 1  # +1因为0是padding
        
        # 过滤正例（这部分仍需要循环，但只循环K次而不是num_items次）
        result = torch.zeros(batch_size, num_neg, dtype=torch.long, device=device)
        ...
        for i in range(batch_size):
            ...
            # 向量化过滤
            valid_mask = torch.tensor([item.item() not in positive_items for item in candidates], device=device)
            valid_candidates = candidates[valid_mask][:num_neg]
            
            if len(valid_candidates) >= num_neg:
                result[i] = valid_candidates[:num_neg]
            else:
                result[i, :len(valid_candidates)] = valid_candidates
                # 补充随机
                result[i, len(valid_candidates):] = torch.randint(1, self.num_items, (num_neg - len(valid_candidates),), device=device)
    ...
    def train_epoch(self, dataloader, optimizer, epoch, total_epochs, all_item_vectors, scheduler=None):
        ...
        for batch in pbar:
            ...
            if config['main_strategy'] == 'inc':
                main_neg = self.neg_sampler.inc_negative_vectorized(user_ids, main_num_neg, self.device)
            elif config['main_strategy'] == 'inbatch':
                main_neg = self.neg_sampler.inbatch_negative_vectorized(item_ids, main_num_neg)
            else:
                main_neg = self.neg_sampler.hard_negative_vectorized(
...
def main(sample_ratio=1.0, epochs=20, temperature=0.05, lr=0.001):
    ...
    # 使用V2版负采样器
    neg_sampler = NegativeSamplerV2(num_items, dict(user_inc_items), dict(user_positive_items), num_users, device)
    trainer = TrainerV2(model, neg_sampler, device, temperature=temperature)
    ...
    torch.save(model.state_dict(), '/home/z/my-project/dssm_recall/checkpoints/model_optimized_v2.pt')
    
    with open('/home/z/my-project/dssm_recall/logs/training_history_optimized_v2.json', 'w') as f:
        json.dump(history, f, indent=2)
    
    with open('/home/z/my-project/dssm_recall/logs/evaluation_results_optimized_v2.json', 'w') as f:
